		...
		return () -> {
			...
			if (exerciciosService.findAll().size() == 0) {
				...
				exerciciosService.create(new Exercicios("Abdominais", "Iniciante", 2, 10,
						...
						new String(((InputStream) getClass().getResourceAsStream("/images/abdominais.txt")).readAllBytes()), new String(((InputStream) getClass().getResourceAsStream("/videos/abdominais.txt")).readAllBytes())));
